...
ATEMmin::ATEMmin(){}
		...
		void ATEMmin::_parseGetCommands(const char *cmdStr)	{
			...
			if(!strcmp_P(cmdStr, PSTR("TlIn"))) {
				// Byte 0-1 of command data is the source count
				sources = word(_packetBuffer[0], _packetBuffer[1]);
				
				// Commented out to clean up serial console as requested
				/*
				if(_serialOutput) {
					Serial.print(F("TlIn Sources: "));
					Serial.println(sources);
					Serial.print(F("Flags[0-2]: "));
					Serial.print(_packetBuffer[2]); Serial.print(" ");
					Serial.print(_packetBuffer[3]); Serial.print(" ");
					Serial.println(_packetBuffer[4]);
				}
				*/
