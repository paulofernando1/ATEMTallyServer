...
void TallyServer::runLoop() {
    ...
    while ((packetSize = _udp.parsePacket()) > 0) {
        if (_udp.available()) {
            ...
            uint16_t packetLen = ((_buffer[0] & 0b00000111) << 8) | _buffer[1];
...
uint16_t TallyServer::_createTallyDataCmd() {
    uint16_t payloadLen = 10 + _atemTallySources;
    uint16_t cmdLen = (payloadLen + 3) & ~3; // 4-byte alignment mandatory for ATEM protocol
    ...
    for(int i = payloadLen; i < cmdLen; i++) {
        _buffer[12 + i] = 0;
    }
