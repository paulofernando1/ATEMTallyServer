...
void ATEMbase::connect(const boolean useFixedPortNumber) {
	...
	_packetBuffer[13] = _tallyID; // CUSTOM: Send Tally ID to Server
...
void ATEMbase::sendRepeaterClients(IPAddress* clients, int8_t* roles, uint8_t count) {
	...
	_wipeCleanPacketBuffer();
	_createCommandHeader(ATEM_headerCmd_AckRequest, 12 + cmdLen);
	
	// Command Segment
	_packetBuffer[12] = highByte(cmdLen);
	_packetBuffer[13] = lowByte(cmdLen);
	_packetBuffer[14] = 0; // Padding
	_packetBuffer[15] = 0; // Padding
	_packetBuffer[16] = 'C';
	_packetBuffer[17] = 'l';
	_packetBuffer[18] = 'n';
	_packetBuffer[19] = 't';
